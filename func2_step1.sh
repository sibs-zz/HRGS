rm *snp *gblup *dl *SNP *pre t1 *dat *txt tmp.recode.vcf cb* order
...
ls *SNP|while read id;do python dl_pre.py $id ${id}.dl;done 
ls *SNP|while read id;do python gblup_pre.py $id ${id}.gblup;done 

ls *SNP|sed s/.SNP//g > col
