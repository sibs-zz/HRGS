...
fit_BRR=BGLR(y=y$danzhuchanliang,ETA=ETA,nIter=nIter,burnIn=burnIn,thin=thin,saveAt=saveAt,df0=5,S0=S0,weights=weights,R2=R2,verbose=F)
#plot(fit_BRR$yHat,y)
#cor(fit_BRR$yHat,y)
#cor(fit_BRR$yHat[tst],DT$danzhuchanliang[tst])
write.table(fit_BRR$yHat,"6_danzhuchanliang.txt",row.names = T,quote = F)
